...
public class MediaController extends IjkBaseMediaController {
	...
	protected View makeControllerView() {
		int ijkmedia_controller = DoResourcesHelper.getIdentifierByStr("do_polyv_ijkmedia_controller", "layout", DoPolyvConstant.TYPEID);
		mRoot = ((LayoutInflater) mContext.getSystemService(Context.LAYOUT_INFLATER_SERVICE)).inflate(ijkmedia_controller, this);
		return mRoot;
	}

	@Override
	protected void initControllerView(View v) {
		int mediacontroller_play_pause = DoResourcesHelper.getIdentifierByStr("mediacontroller_play_pause", "id", DoPolyvConstant.TYPEID);
		mPauseButton = (ImageButton) v.findViewById(mediacontroller_play_pause);
		if (mPauseButton != null) {
			mPauseButton.requestFocus();
			mPauseButton.setOnClickListener(mPauseListener);
		}

		int ffwd = DoResourcesHelper.getIdentifierByStr("ffwd", "id", DoPolyvConstant.TYPEID);
		mFfwdButton = (ImageButton) v.findViewById(ffwd);
		mFfwdButton.setOnClickListener(mFfwdListener);

		int rew = DoResourcesHelper.getIdentifierByStr("rew", "id", DoPolyvConstant.TYPEID);
		mRewButton = (ImageButton) v.findViewById(rew);
		mRewButton.setOnClickListener(mRewListener);
		int landscape = DoResourcesHelper.getIdentifierByStr("landscape", "id", DoPolyvConstant.TYPEID);
		btn_boardChange = (ImageButton) v.findViewById(landscape);
		btn_boardChange.setOnClickListener(mBoardListener);
		int videochange = DoResourcesHelper.getIdentifierByStr("videochange", "id", DoPolyvConstant.TYPEID);
		btn_videoChange = (ImageButton) v.findViewById(videochange);
		btn_videoChange.setTag("0");
		btn_videoChange.setOnClickListener(mVideoListener);

		int prev = DoResourcesHelper.getIdentifierByStr("prev", "id", DoPolyvConstant.TYPEID);
		mPreButton = (ImageButton) v.findViewById(prev);
		int next = DoResourcesHelper.getIdentifierByStr("next", "id", DoPolyvConstant.TYPEID);
		mNextButton = (ImageButton) v.findViewById(next);
		if (isUsePreNext) {
		...
		int mediacontroller_seekbar = DoResourcesHelper.getIdentifierByStr("mediacontroller_seekbar", "id", DoPolyvConstant.TYPEID);
		mProgress = (ProgressBar) v.findViewById(mediacontroller_seekbar);
		if (mProgress != null) {
		...
		int mediacontroller_time_total = DoResourcesHelper.getIdentifierByStr("mediacontroller_time_total", "id", DoPolyvConstant.TYPEID);
		mEndTime = (TextView) v.findViewById(mediacontroller_time_total);
		int mediacontroller_time_current = DoResourcesHelper.getIdentifierByStr("mediacontroller_time_current", "id", DoPolyvConstant.TYPEID);
		mCurrentTime = (TextView) v.findViewById(mediacontroller_time_current);

		// 码率选择功能涉及的控件
		int select_bitrate = DoResourcesHelper.getIdentifierByStr("select_bitrate", "id", DoPolyvConstant.TYPEID);
		selectBitrate = (Button) mRoot.findViewById(select_bitrate);
		selectBitrate.setOnClickListener(mSelectBitRate);
		int bitrate_linear_layout = DoResourcesHelper.getIdentifierByStr("bitrate_linear_layout", "id", DoPolyvConstant.TYPEID);
		bitrateLinearLayout = (LinearLayout) mRoot.findViewById(bitrate_linear_layout);

		bitRateBtnArray = new SparseArray<Button>();
		int liuchang = DoResourcesHelper.getIdentifierByStr("liuchang", "id", DoPolyvConstant.TYPEID);
		Button liuchangBtn = (Button) mRoot.findViewById(liuchang);
		liuchangBtn.setText(BitRateEnum.liuChang.getName());
		bitRateBtnArray.append(BitRateEnum.liuChang.getNum(), liuchangBtn);

		int gaoqing = DoResourcesHelper.getIdentifierByStr("gaoqing", "id", DoPolyvConstant.TYPEID);
		Button gaoqingBtn = (Button) mRoot.findViewById(gaoqing);
		gaoqingBtn.setText(BitRateEnum.gaoQing.getName());
		bitRateBtnArray.append(BitRateEnum.gaoQing.getNum(), gaoqingBtn);

		int chaoqing = DoResourcesHelper.getIdentifierByStr("chaoqing", "id", DoPolyvConstant.TYPEID);
		Button chaoqingBtn = (Button) mRoot.findViewById(chaoqing);
	...
	public void hide() {
		...
		if (mShowing) {
			...
			try {
				...
				DoServiceContainer.getLogEngine().writeError("MediaController already removed", ex);
			}
			mShowing = false;
	...
	private void updatePausePlay() {
		if (mRoot == null || mPauseButton == null)
			return;
		if (mPlayer.isPlaying()) {
			mPauseButton.setImageResource(DoResourcesHelper.getRIdByDrawable("media_pause", DoPolyvConstant.TYPEID));
		} else {
			mPauseButton.setImageResource(DoResourcesHelper.getRIdByDrawable("media_play", DoPolyvConstant.TYPEID));
		}
	}
